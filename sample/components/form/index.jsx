...
const <SampleSingle>Form = () => {
  ...
  const defaultData = uiEditando ? <sampleSingle>Corrente : DEFAULT_FORM_DATA;
  ...
  useEffect(() => {
    if (uiEditando) setData(JSON.parse(JSON.stringify(<sampleSingle>Corrente)));
  }, [<sampleSingle>Corrente]);
  ...
  const obterTituloForm = () => {
    ...
    return t(key, { model: t('<i18nNamespace>:<samplePlural>.model.one') });
  };
...
export default <SampleSingle>Form;
